...
public class Empty_Red_Brazier : MonoBehaviour
{
    ...
    public GameObject associatedRedBrazier;
    ...
    void Update()
    {
        ...
        if(associatedRedBrazier.GetComponent<BrazierBehaviour>().alreadyPickedUp && canDropOff)
        {
            if (!this.GetComponent<BrazierBehaviour>().alreadyDroppedOff)
            {
                player.GetComponent<Player_Controller>().shouldIDisplayQText = true;
            }
        }
    }
