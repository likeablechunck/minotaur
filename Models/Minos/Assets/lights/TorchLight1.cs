...
{
    public class TorchLight1 : MonoBehaviour
    {
        private float m_Rnd;
        ...
        private void Update()
        {
            if (m_Burning)
            {
                m_Light.intensity = 2*Mathf.PerlinNoise(m_Rnd + Time.time, m_Rnd + 4 + Time.time*.3f)+9;
               /* if (m_Light.intensity < 1.2f){
                ...
                transform.localPosition = Vector3.up + new Vector3(x, y-2.0f, z)*.3f;
            }
        }
